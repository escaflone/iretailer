...
left join (
    <#include "inout.ftl"/>
) `inout` on `inout`._d = <#include "column/timelineDate.ftl"/>
</#if>
<#--passby 是否需要-->
<#if column.count_passby ??>
left join (
    <#include "passby.ftl"/>
) `passby` on `passby`._d = <#include "column/timelineDate.ftl"/>
</#if>
<#--sale 是否需要-->
<#if column.count_sales ?? || column.count_goods ?? || column.count_trades ??>
left join (
    <#include "sale.ftl"/>
) `sale` on `sale`._d = <#include "column/timelineDate.ftl"/>
</#if>
